...
# Load the data
data = xr.open_dataset('tas/map.nc')
pvalues = xr.open_dataset('tas/pvalue.nc')

# Extract variables
data_values = data['tas_trend']  # Main data variable
pvalue_values = pvalues['tas_trend_pvalue']  # P-value variable
...
cmap = plt.get_cmap('coolwarm', 12)  # 10 discrete colors
norm = BoundaryNorm(np.linspace(-1.5, 1.5, 13), cmap.N)  # Define boundaries for 10 intervals
...
cbar.set_label('Temperature Trend (deg C per decade)')

# Add titles and labels
ax.set_title('ERA5 - Mean temperature (T) Trend deg C per decade - 1980-2015 - Annual', pad=15)

# Save the figure as PNG with high resolution and no extra white space
plt.savefig('temperature_trend_central_asia.png', dpi=300, bbox_inches='tight', pad_inches=0)
